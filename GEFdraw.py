...
true_file = 'GEFCHformer/true.npy'
styles = {
...
for V in range(3):
    ...
    plt.tight_layout()
    # 保存+显示
    plt.savefig('GEF' + str(V) + '.png', dpi=300, bbox_inches='tight')
    plt.close()  # 关闭画布释放内存
...
# 可选：保存误差结果到CSV文件
error_df.to_csv('GEFmetric.csv', index=False, encoding='utf-8-sig')
